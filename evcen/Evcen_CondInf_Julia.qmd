...
contrasts = merge(
     ...
     Dict(j => EffectsCoding() for j in (:Type, :Cat, :Resp))
     )
```
...
m_ovi1 = let
    form = @formula(log(rt) ~ 1 + (Type + Cat + Resp)^1 + (1 | Subj) + (1 | Item));
...
m_ovi2 = let
    form = @formula(log(rt) ~ 1 + (Type + Cat + Resp)^2 + (1 | Subj) + (1 | Item));
    fit(MixedModel, form, dat; contrasts);
  end
```
...
m_ovi3 = let
    form = @formula(log(rt) ~ 1 + (Type + Cat + Resp)^3 + (1 | Subj) + (1 | Item));
    fit(MixedModel, form, dat; contrasts);
  end
```
...
m_cpx = let
    form = @formula(log(rt) ~ 1 + (Type + Cat + Resp)^2 + (1 + Type + Cat + Resp | Subj) + (1 + Cat + Resp | Item));
    fit(MixedModel, form, dat; contrasts);
  end
...
m_zcp = let
    form = @formula(log(rt) ~ 1 + (Type + Cat + Resp)^2 + 
                    zerocorr(1 + Type + Cat + Resp | Subj) + zerocorr(1 + Cat + Resp | Item));
    fit(MixedModel, form, dat; contrasts);
  end
...
shrinkageplot!(Figure(; resolution=(1000, 1200)), m_prm1, :Subj)
```
```{julia}
#| code-fold: true
